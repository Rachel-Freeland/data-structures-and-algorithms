...
const addPurchases = (arr) => {
  let totalPrice = arr.reduce( (total, value) => total + value.purchasePrice, 0 );
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  let length = arr.reduce( (acc, i) => acc + 1, 0);
  return length;
};
...
const returnNames = (arr) => {
  let answer = [];
  arr.reduce( (acc, value) => answer.push(value.name), [] );
  return answer;
};
...
const reversedString = (str) => {
  let arr = str.split('');
  let answer = arr.reduce( (acc, currentChar) => {
    return currentChar + acc;}, '');
  return answer;
};
