...
const maxInArray = (arr) => {
  return arr.reduce( (n1, n2) => n1 >= n2 ? n1 : n2);
};
...
const getHouses = (arr) => {
  let houses = [];
  arr.map( character => houses.push(character.house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => Object.keys( arr.find( char => char.name === character)).includes('children');
