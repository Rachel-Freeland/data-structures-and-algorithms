...
const longestString = (arr) => {
  let answer = '';
  arr.forEach( string => string.length > answer.length ? answer = string : answer );
...
const onlyOddChars = (str) => {
  let answer =[];
  str.split('').reduce( (acc, currValue, i) => i%2 === 1 ? answer.push(currValue) : '', []);
  return answer.join('');
};
...
const allHappy = (arr) => {
  return arr.length === arr.filter( string => string.includes(':)')).length;
};
