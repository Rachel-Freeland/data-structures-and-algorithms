...
const count = (target, input) => {
  let answer = 0;
  input.map( arr => {
    answer += arr.filter( num => num === target).length;
  });
  return answer;
};
...
const totalSum = (input) => {
  let answer = 0;
  input.map( arr => answer += arr.reduce((acc, currvalue) => acc += currvalue, 0));
  return answer;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map( arr => arr.filter( value => typeof value === 'number' && value %5 === 0).map(num => Math.pow(2, num)));
};
...
let findShortest = (data) => {
  return data.reduce((char1, char2) => Number(char1.height) < Number(char2.height) ? char1 : char2).name;
};
