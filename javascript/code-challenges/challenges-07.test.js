...
const sortStarWarsCharacters = (starWarsArr) => starWarsArr.sort( (a, b) => Number(b.height) - Number(a.height));
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach( ingredient => {
    let firstCut = ingredient.slice(ingredient.indexOf(' ') + 1);
    result.push(firstCut.slice(firstCut.indexOf(' ') + 1));
  });
  return result;
};
